    ...
    def forward(self, result, writer, step):
        ...
        def _hook(name, truth_a, truth_c, is_ant):
            ...
            def hook(grad):
                ...
                writer.add_scalar(tag + 'correct_update', tot_corr_up / tot_abs_grad, step)

                tot_corr_reas = torch.sum(abs_grad[corr_reason])
                writer.add_scalar(tag + 'correct_reason', tot_corr_reas / tot_abs_grad, step)
        ...
        def same_hook(grad):
            ...
            writer.add_scalar('same/precision', torch.sum(grad[correct]) / torch.sum(grad), step)
        ...
        if writer:
            ...
            labels_same_t = labels_same.view(n, n).transpose(0, 1).contiguous().view(n * n)
